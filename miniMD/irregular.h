...
namespace FCPU
{
	...
	DEVICE bool map1(OBJECT *object, void *point_data, int *part_index, void *edge_data, void *parameter, unsigned int task_id, unsigned int part_id, irreduce_fp reduce_ptr)
	{
		double cutforcesq = *(double *)parameter;

		double *input = (double *)point_data;

		int *edges = (int *)edge_data;

		double xx,yy,zz;
		double rd;
		double rrd, rrd2, rrd3;
		VALUE forces;
		double force;
		...
		xx = input[3 * i] - input[3 * j];
		yy = input[3 * i + 1] - input[3 * j + 1];
		zz = input[3 * i + 2] - input[3 * j + 2];
	...
	CONSTANT irmap_fp map_function_table[] = {map1};
