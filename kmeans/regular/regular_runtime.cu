...
void RegularRuntime::RegularStart()
{
	...
	cudaMalloc(&p1[0],R_GPU_PREALLOC_SIZE * sizeof(Offset));
	cudaMalloc(&p2[0],R_GPU_PREALLOC_SIZE * sizeof(float)*3);
	CUDA_SAFE_CALL(cudaSetDevice(1));
	cudaMalloc(&p1[1],R_GPU_PREALLOC_SIZE * sizeof(Offset));
	cudaMalloc(&p2[1],R_GPU_PREALLOC_SIZE * sizeof(float)*3);
	...
	while(*task_offset_ < total_offsets)
	{
		for(int i = 0; i < num_gpus_; i++)
		{
			...
			*task_offset_ += R_GPU_PREALLOC_SIZE; 
			...
			CUDA_SAFE_CALL(cudaMemcpyAsync(p1[i], offsets_pin_,R_GPU_PREALLOC_SIZE * sizeof(Offset) , cudaMemcpyHostToDevice));
			CUDA_SAFE_CALL(cudaMemcpyAsync(p2[i], input_pin_, R_GPU_PREALLOC_SIZE * sizeof(float)*3, cudaMemcpyHostToDevice));
