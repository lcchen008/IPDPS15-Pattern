...
void process_a_tile(int num_dims, int unit_size, void *input, void *output, struct Tile &tile, IndexArray &size, void *parameter, stencil_fp stencil_ptr)
{
	IndexArray offset_tmp = tile.offset;
	...
	if(num_dims == 2)	
	{
		for(int i = 0; i < tile.size[1]; i++)	
			for(int j = 0; j < tile.size[0]; j++)
			{
				...
				stencil_ptr(input, output, (int *)&offset_tmp[0], (int *)&size[0], parameter);
			}
	}

	else if(num_dims == 3)
	{
		for(int i = 0; i < tile.size[2]; i++, offset_tmp[2]++)	
		{
			for(int j = 0; j < tile.size[1]; j++, offset_tmp[1]++)
			{
				for(int k = 0; k < tile.size[0]; k++, offset_tmp[0]++)
				{
					stencil_ptr(input, output, (int *)&offset_tmp[0], (int *)&size[0], parameter);
				}
			}
...
void *compute_tiles(void *arg)
{
	...
	void *parameter = runtime->get_parameter();
	...
	for(int i = start; i < end; i++)
	{
		...
		process_a_tile(num_dims, unit_size, input, output, tile, total_size, parameter, stencil_ptr);
	}
